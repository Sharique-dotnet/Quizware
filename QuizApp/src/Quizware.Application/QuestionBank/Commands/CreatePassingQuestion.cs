...
public sealed record CreatePassingQuestionCommand(
    ...
    int MaxPassCount, PassDirection PassDirection, bool RevealAnswerIfAllPass, Guid? ReplacesQuestionId = null) : IRequest<QuestionDto>;

public sealed class CreatePassingQuestionCommandValidator : AbstractValidator<CreatePassingQuestionCommand>
{
    public CreatePassingQuestionCommandValidator()
    {
...
public sealed class CreatePassingQuestionCommandHandler : IRequestHandler<CreatePassingQuestionCommand, QuestionDto>
{
    private readonly IAppDbContext _db;
    private readonly ICurrentUser _currentUser;

    public CreatePassingQuestionCommandHandler(IAppDbContext db, ICurrentUser currentUser)
    {
        _db = db;
        _currentUser = currentUser;
    }

    public async Task<QuestionDto> Handle(CreatePassingQuestionCommand request, CancellationToken cancellationToken)
    {
        ...
        var previousVersion = await QuestionCommon.ResolveReplacementTargetAsync(
            _db, request.ProgramId, request.ReplacesQuestionId, QuestionFormatCode.Passing, cancellationToken);

        var actor = _currentUser.Email ?? "unknown";
        var question = PassingQuestion.Create(
            ...
            request.MaxPassCount, request.PassDirection, request.RevealAnswerIfAllPass);
